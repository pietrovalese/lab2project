		...
		try:
			...
			with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor: #crea e gestisce multithreads
				while True:
					...
					executor.submit(gestisci_connessione,connection,addr,capolet,caposc)#faccio gestire ai thread la connessione
...
if __name__ == '__main__': #verifica se il programma viene eseguito come script principale
	parser = argparse.ArgumentParser(description=Description, formatter_class=argparse.RawTextHelpFormatter)
	parser.add_argument('t', help='numero thread', type = int)    #numero thread
	parser.add_argument('-r', help='reader archivio', type = str, default=3) #numero thread reader di archivio
	parser.add_argument('-w', help='writer archivio', type = str, default=3) #numero thread writer di archivio
	parser.add_argument('-v', help='launch valgrind', nargs='?', const='call_-v')    #nargs indefinito, assume il valore di const se viene scritto -v senza parametri
